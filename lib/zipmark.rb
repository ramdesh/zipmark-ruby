...
class ZipMark
  ...
  def build_header_auth(uri, version)
  ...
  def get_approval_rules()
    build_header_auth('/approval_rules', API_VERSION_1)
    build_request()
  ...
  def get_vendor_relationships()
    build_header_auth('/vendor_relationships', API_VERSION_1)
    build_request()
    request = Net::HTTP::Get.new(@uri.request_uri)
    @header.each do |name, value|
      request[name] = value
  ...
  def create_bill(params)
    build_header_auth('/bills', API_VERSION_1)
    build_request()
    p_hash = {}
    ...
    def bill_callback(params, api_version)
      if version == API_VERSION_1
        build_header_auth('/callback', 'POST')
        build_request()
        p_hash = {}
        p_hash["callback"] = {
          "amount_cents"    => params['amount_cents'],
        ...
        build_header_auth('/callbacks', 'POST')
        build_request()
        p_hash = {}
        p_hash["callback"] = {
          "amount_cents"    => params['amount_cents'],
